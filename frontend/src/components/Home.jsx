    ...
    <>
      ...
      {result?.result  && (
        <div className="absolute bottom-4 left-2 right-2 sm:left-10 sm:right-auto bg-black/80 backdrop-blur-md text-white p-4 sm:p-6 rounded-xl shadow-lg z-20 max-w-md border border-gray-700 mx-auto sm:mx-0">
          <h3 className="text-lg sm:text-xl font-semibold mb-2 text-white/90">Result</h3>
          <div className="space-y-2 sm:space-y-3 text-sm sm:text-base">
            <p>
              <span className="font-medium text-gray-300">Expression:</span>{" "}
              <span className="font-mono text-blue-300">{result.result.recognized_expression}</span>
            </p>

            <p>
              <span className="font-medium text-gray-300">Solution:</span>{" "}
              <span className="font-mono text-green-300 text-lg">{result.result.solution}</span>
            </p>
            ...
            <p className="text-gray-400">
              ...
              {result.result.type?.charAt(0).toUpperCase() + result.result.type?.slice(1)}
            </p>
          </div>
        </div>
      )}
